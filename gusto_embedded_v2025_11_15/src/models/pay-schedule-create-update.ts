...
} from "./pay-schedule-frequency-create-update.js";
...
/** @internal */
export const PayScheduleCreateUpdate$inboundSchema: z.ZodMiniType<
  PayScheduleCreateUpdate,
  unknown
> = z.pipe(
  z.object({
    uuid: types.string(),
    frequency: types.optional(PayScheduleFrequencyCreateUpdate$inboundSchema),
...
export function payScheduleCreateUpdateFromJSON(
  jsonString: string,
): SafeParseResult<PayScheduleCreateUpdate, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => PayScheduleCreateUpdate$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'PayScheduleCreateUpdate' from JSON`,
  );
}
