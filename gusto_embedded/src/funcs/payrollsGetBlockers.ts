...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetV1CompaniesPayrollBlockersCompanyUuidRequest$outboundSchema
        .parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.PayrollBlocker$inboundSchema)),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
