 ...
 */
export async function companyAttachmentsGetList(
  client: GustoEmbeddedCore,
  request: operations.GetV1CompaniesAttachmentsRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<components.CompanyAttachment>,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetV1CompaniesAttachmentsRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.CompanyAttachment$inboundSchema)),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
