 ...
 */
export async function timeOffPoliciesGetAll(
  client: GustoEmbeddedCore,
  request: operations.GetCompaniesCompanyUuidTimeOffPoliciesRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<components.TimeOffPolicy>,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetCompaniesCompanyUuidTimeOffPoliciesRequest$outboundSchema
        .parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.TimeOffPolicy$inboundSchema)),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
