...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetV1PaymentReceiptsPayrollsPayrollUuidRequest$outboundSchema
        .parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.PayrollReceipt$inboundSchema),
    M.fail([404, "4XX", "5XX"]),
  )(response);
  if (!result.ok) {
    return result;
