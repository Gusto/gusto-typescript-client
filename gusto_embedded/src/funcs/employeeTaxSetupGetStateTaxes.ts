 ...
 */
export async function employeeTaxSetupGetStateTaxes(
  client: GustoEmbeddedCore,
  request: operations.GetV1EmployeesEmployeeIdStateTaxesRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<components.EmployeeStateTax>,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetV1EmployeesEmployeeIdStateTaxesRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.EmployeeStateTax$inboundSchema)),
    M.fail([404, "4XX", "5XX"]),
  )(response);
  if (!result.ok) {
    return result;
