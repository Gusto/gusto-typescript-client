 ...
 */
export async function employeeEmploymentsGetHistory(
  client: GustoEmbeddedCore,
  request: operations.GetV1EmployeesEmployeeIdEmploymentHistoryRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<components.EmploymentHistoryList>,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.EmploymentHistoryList$inboundSchema)),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
