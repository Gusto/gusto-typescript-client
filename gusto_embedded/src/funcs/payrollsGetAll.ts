 ...
 * scope: `payrolls:read`
 */
export async function payrollsGetAll(
  client: GustoEmbeddedCore,
  request: operations.GetV1CompaniesCompanyIdPayrollsRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<components.PayrollMinimal>,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetV1CompaniesCompanyIdPayrollsRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.PayrollMinimal$inboundSchema)),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
