...
* [getAll](#getall) - Get all contractor bank accounts
...
async function run() {
  const result = await gustoEmbedded.contractorPaymentMethods.getAll({
    contractorUuid: "<id>",
  });
...
import { contractorPaymentMethodsGetAll } from "gusto_embedded/funcs/contractorPaymentMethodsGetAll.js";
...
async function run() {
  const res = await contractorPaymentMethodsGetAll(gustoEmbedded, {
    contractorUuid: "<id>",
  });
