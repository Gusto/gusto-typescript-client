...
async function run() {
  const result = await gustoEmbedded.companies.contractors.list({
    companyUuid: "<id>",
  });
...
import { companiesContractorsList } from "gusto_embedded/funcs/companiesContractorsList.js";
...
async function run() {
  const res = await companiesContractorsList(gustoEmbedded, {
    companyUuid: "<id>",
  });
