...
exports.payrollsList = payrollsList;
 ...
 * scope: `payrolls:read`
 */
function payrollsList(client, request, options) {
    return new async_js_1.APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = (0, schemas_js_1.safeParse)(request, (value) => operations.GetV1CompaniesCompanyIdPayrollsRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(200, operations.GetV1CompaniesCompanyIdPayrollsResponse$inboundSchema, { key: "Payroll-List" }), M.fail([404, "4XX"]), M.fail("5XX"))(response, { extraFields: responseFields });
    if (!result.ok) {
