/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v4-mini";
import { remap as remap$ } from "../../lib/primitives.js";
import { ClosedEnum } from "../../types/enums.js";

/**
 * Determines the date-based API version associated with your API call. If none is provided, your application's [minimum API version](https://docs.gusto.com/embedded-payroll/docs/api-versioning#minimum-api-version) is used.
 */
export const GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdXGustoAPIVersion =
  {
    TwoThousandAndTwentyFiveMinus11Minus15: "2025-11-15",
  } as const;
/**
 * Determines the date-based API version associated with your API call. If none is provided, your application's [minimum API version](https://docs.gusto.com/embedded-payroll/docs/api-versioning#minimum-api-version) is used.
 */
export type GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdXGustoAPIVersion =
  ClosedEnum<
    typeof GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdXGustoAPIVersion
  >;

export type GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdRequest = {
  /**
   * Determines the date-based API version associated with your API call. If none is provided, your application's [minimum API version](https://docs.gusto.com/embedded-payroll/docs/api-versioning#minimum-api-version) is used.
   */
  xGustoAPIVersion?:
    | GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdXGustoAPIVersion
    | undefined;
  /**
   * The UUID of the company
   */
  companyId: string;
  /**
   * The UUID of the pay schedule
   */
  payScheduleId: string;
};

/** @internal */
export const GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdXGustoAPIVersion$outboundSchema:
  z.ZodMiniEnum<
    typeof GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdXGustoAPIVersion
  > = z.enum(GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdXGustoAPIVersion);

/** @internal */
export type GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdRequest$Outbound = {
  "X-Gusto-API-Version": string;
  company_id: string;
  pay_schedule_id: string;
};

/** @internal */
export const GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdRequest$outboundSchema:
  z.ZodMiniType<
    GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdRequest$Outbound,
    GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdRequest
  > = z.pipe(
    z.object({
      xGustoAPIVersion: z._default(
        GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdXGustoAPIVersion$outboundSchema,
        "2025-11-15",
      ),
      companyId: z.string(),
      payScheduleId: z.string(),
    }),
    z.transform((v) => {
      return remap$(v, {
        xGustoAPIVersion: "X-Gusto-API-Version",
        companyId: "company_id",
        payScheduleId: "pay_schedule_id",
      });
    }),
  );

export function getV1CompaniesCompanyIdPaySchedulesPayScheduleIdRequestToJSON(
  getV1CompaniesCompanyIdPaySchedulesPayScheduleIdRequest:
    GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdRequest,
): string {
  return JSON.stringify(
    GetV1CompaniesCompanyIdPaySchedulesPayScheduleIdRequest$outboundSchema
      .parse(getV1CompaniesCompanyIdPaySchedulesPayScheduleIdRequest),
  );
}
