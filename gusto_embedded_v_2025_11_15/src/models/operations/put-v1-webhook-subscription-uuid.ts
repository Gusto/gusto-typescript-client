/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v4-mini";
import { remap as remap$ } from "../../lib/primitives.js";
import { ClosedEnum } from "../../types/enums.js";

export type PutV1WebhookSubscriptionUuidSecurity = {
  systemAccessAuth: string;
};

/**
 * Determines the date-based API version associated with your API call. If none is provided, your application's [minimum API version](https://docs.gusto.com/embedded-payroll/docs/api-versioning#minimum-api-version) is used.
 */
export const PutV1WebhookSubscriptionUuidXGustoAPIVersion = {
  TwoThousandAndTwentyFiveMinus11Minus15: "2025-11-15",
} as const;
/**
 * Determines the date-based API version associated with your API call. If none is provided, your application's [minimum API version](https://docs.gusto.com/embedded-payroll/docs/api-versioning#minimum-api-version) is used.
 */
export type PutV1WebhookSubscriptionUuidXGustoAPIVersion = ClosedEnum<
  typeof PutV1WebhookSubscriptionUuidXGustoAPIVersion
>;

export const PutV1WebhookSubscriptionUuidSubscriptionType = {
  BankAccount: "BankAccount",
  Company: "Company",
  CompanyBenefit: "CompanyBenefit",
  Contractor: "Contractor",
  ContractorPayment: "ContractorPayment",
  Employee: "Employee",
  EmployeeBenefit: "EmployeeBenefit",
  EmployeeJobCompensation: "EmployeeJobCompensation",
  ExternalPayroll: "ExternalPayroll",
  Form: "Form",
  Location: "Location",
  Notification: "Notification",
  Payroll: "Payroll",
  PayrollSync: "PayrollSync",
  PaySchedule: "PaySchedule",
  Signatory: "Signatory",
} as const;
export type PutV1WebhookSubscriptionUuidSubscriptionType = ClosedEnum<
  typeof PutV1WebhookSubscriptionUuidSubscriptionType
>;

export type PutV1WebhookSubscriptionUuidRequestBody = {
  /**
   * The types of events to subscribe to.
   */
  subscriptionTypes: Array<PutV1WebhookSubscriptionUuidSubscriptionType>;
};

export type PutV1WebhookSubscriptionUuidRequest = {
  /**
   * The webhook subscription UUID.
   */
  webhookSubscriptionUuid: string;
  /**
   * Determines the date-based API version associated with your API call. If none is provided, your application's [minimum API version](https://docs.gusto.com/embedded-payroll/docs/api-versioning#minimum-api-version) is used.
   */
  xGustoAPIVersion?: PutV1WebhookSubscriptionUuidXGustoAPIVersion | undefined;
  body: PutV1WebhookSubscriptionUuidRequestBody;
};

/** @internal */
export type PutV1WebhookSubscriptionUuidSecurity$Outbound = {
  SystemAccessAuth: string;
};

/** @internal */
export const PutV1WebhookSubscriptionUuidSecurity$outboundSchema: z.ZodMiniType<
  PutV1WebhookSubscriptionUuidSecurity$Outbound,
  PutV1WebhookSubscriptionUuidSecurity
> = z.pipe(
  z.object({
    systemAccessAuth: z.string(),
  }),
  z.transform((v) => {
    return remap$(v, {
      systemAccessAuth: "SystemAccessAuth",
    });
  }),
);

export function putV1WebhookSubscriptionUuidSecurityToJSON(
  putV1WebhookSubscriptionUuidSecurity: PutV1WebhookSubscriptionUuidSecurity,
): string {
  return JSON.stringify(
    PutV1WebhookSubscriptionUuidSecurity$outboundSchema.parse(
      putV1WebhookSubscriptionUuidSecurity,
    ),
  );
}

/** @internal */
export const PutV1WebhookSubscriptionUuidXGustoAPIVersion$outboundSchema:
  z.ZodMiniEnum<typeof PutV1WebhookSubscriptionUuidXGustoAPIVersion> = z.enum(
    PutV1WebhookSubscriptionUuidXGustoAPIVersion,
  );

/** @internal */
export const PutV1WebhookSubscriptionUuidSubscriptionType$outboundSchema:
  z.ZodMiniEnum<typeof PutV1WebhookSubscriptionUuidSubscriptionType> = z.enum(
    PutV1WebhookSubscriptionUuidSubscriptionType,
  );

/** @internal */
export type PutV1WebhookSubscriptionUuidRequestBody$Outbound = {
  subscription_types: Array<string>;
};

/** @internal */
export const PutV1WebhookSubscriptionUuidRequestBody$outboundSchema:
  z.ZodMiniType<
    PutV1WebhookSubscriptionUuidRequestBody$Outbound,
    PutV1WebhookSubscriptionUuidRequestBody
  > = z.pipe(
    z.object({
      subscriptionTypes: z.array(
        PutV1WebhookSubscriptionUuidSubscriptionType$outboundSchema,
      ),
    }),
    z.transform((v) => {
      return remap$(v, {
        subscriptionTypes: "subscription_types",
      });
    }),
  );

export function putV1WebhookSubscriptionUuidRequestBodyToJSON(
  putV1WebhookSubscriptionUuidRequestBody:
    PutV1WebhookSubscriptionUuidRequestBody,
): string {
  return JSON.stringify(
    PutV1WebhookSubscriptionUuidRequestBody$outboundSchema.parse(
      putV1WebhookSubscriptionUuidRequestBody,
    ),
  );
}

/** @internal */
export type PutV1WebhookSubscriptionUuidRequest$Outbound = {
  webhook_subscription_uuid: string;
  "X-Gusto-API-Version": string;
  body: PutV1WebhookSubscriptionUuidRequestBody$Outbound;
};

/** @internal */
export const PutV1WebhookSubscriptionUuidRequest$outboundSchema: z.ZodMiniType<
  PutV1WebhookSubscriptionUuidRequest$Outbound,
  PutV1WebhookSubscriptionUuidRequest
> = z.pipe(
  z.object({
    webhookSubscriptionUuid: z.string(),
    xGustoAPIVersion: z._default(
      PutV1WebhookSubscriptionUuidXGustoAPIVersion$outboundSchema,
      "2025-11-15",
    ),
    body: z.lazy(() => PutV1WebhookSubscriptionUuidRequestBody$outboundSchema),
  }),
  z.transform((v) => {
    return remap$(v, {
      webhookSubscriptionUuid: "webhook_subscription_uuid",
      xGustoAPIVersion: "X-Gusto-API-Version",
    });
  }),
);

export function putV1WebhookSubscriptionUuidRequestToJSON(
  putV1WebhookSubscriptionUuidRequest: PutV1WebhookSubscriptionUuidRequest,
): string {
  return JSON.stringify(
    PutV1WebhookSubscriptionUuidRequest$outboundSchema.parse(
      putV1WebhookSubscriptionUuidRequest,
    ),
  );
}
