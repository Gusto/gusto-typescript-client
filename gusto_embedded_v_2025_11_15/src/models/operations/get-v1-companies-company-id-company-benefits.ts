/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v4-mini";
import { remap as remap$ } from "../../lib/primitives.js";
import { ClosedEnum } from "../../types/enums.js";

/**
 * Determines the date-based API version associated with your API call. If none is provided, your application's [minimum API version](https://docs.gusto.com/embedded-payroll/docs/api-versioning#minimum-api-version) is used.
 */
export const GetV1CompaniesCompanyIdCompanyBenefitsXGustoAPIVersion = {
  TwoThousandAndTwentyFiveMinus11Minus15: "2025-11-15",
} as const;
/**
 * Determines the date-based API version associated with your API call. If none is provided, your application's [minimum API version](https://docs.gusto.com/embedded-payroll/docs/api-versioning#minimum-api-version) is used.
 */
export type GetV1CompaniesCompanyIdCompanyBenefitsXGustoAPIVersion = ClosedEnum<
  typeof GetV1CompaniesCompanyIdCompanyBenefitsXGustoAPIVersion
>;

export type GetV1CompaniesCompanyIdCompanyBenefitsRequest = {
  /**
   * Determines the date-based API version associated with your API call. If none is provided, your application's [minimum API version](https://docs.gusto.com/embedded-payroll/docs/api-versioning#minimum-api-version) is used.
   */
  xGustoAPIVersion?:
    | GetV1CompaniesCompanyIdCompanyBenefitsXGustoAPIVersion
    | undefined;
  /**
   * The UUID of the company
   */
  companyId: string;
  /**
   * Whether the benefit is currently active
   */
  active?: boolean | undefined;
  /**
   * Whether to return employee enrollment count
   */
  enrollmentCount?: boolean | undefined;
  /**
   * Filter by benefit type. Comma-separated list of benefit type IDs, i.e. `?benefit_type=5,105`
   */
  benefitType?: string | undefined;
};

/** @internal */
export const GetV1CompaniesCompanyIdCompanyBenefitsXGustoAPIVersion$outboundSchema:
  z.ZodMiniEnum<typeof GetV1CompaniesCompanyIdCompanyBenefitsXGustoAPIVersion> =
    z.enum(GetV1CompaniesCompanyIdCompanyBenefitsXGustoAPIVersion);

/** @internal */
export type GetV1CompaniesCompanyIdCompanyBenefitsRequest$Outbound = {
  "X-Gusto-API-Version": string;
  company_id: string;
  active?: boolean | undefined;
  enrollment_count?: boolean | undefined;
  benefit_type?: string | undefined;
};

/** @internal */
export const GetV1CompaniesCompanyIdCompanyBenefitsRequest$outboundSchema:
  z.ZodMiniType<
    GetV1CompaniesCompanyIdCompanyBenefitsRequest$Outbound,
    GetV1CompaniesCompanyIdCompanyBenefitsRequest
  > = z.pipe(
    z.object({
      xGustoAPIVersion: z._default(
        GetV1CompaniesCompanyIdCompanyBenefitsXGustoAPIVersion$outboundSchema,
        "2025-11-15",
      ),
      companyId: z.string(),
      active: z.optional(z.boolean()),
      enrollmentCount: z.optional(z.boolean()),
      benefitType: z.optional(z.string()),
    }),
    z.transform((v) => {
      return remap$(v, {
        xGustoAPIVersion: "X-Gusto-API-Version",
        companyId: "company_id",
        enrollmentCount: "enrollment_count",
        benefitType: "benefit_type",
      });
    }),
  );

export function getV1CompaniesCompanyIdCompanyBenefitsRequestToJSON(
  getV1CompaniesCompanyIdCompanyBenefitsRequest:
    GetV1CompaniesCompanyIdCompanyBenefitsRequest,
): string {
  return JSON.stringify(
    GetV1CompaniesCompanyIdCompanyBenefitsRequest$outboundSchema.parse(
      getV1CompaniesCompanyIdCompanyBenefitsRequest,
    ),
  );
}
