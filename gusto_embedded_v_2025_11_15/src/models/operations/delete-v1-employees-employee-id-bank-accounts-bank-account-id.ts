/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v4-mini";
import { remap as remap$ } from "../../lib/primitives.js";
import { ClosedEnum } from "../../types/enums.js";

/**
 * Determines the date-based API version associated with your API call. If none is provided, your application's [minimum API version](https://docs.gusto.com/embedded-payroll/docs/api-versioning#minimum-api-version) is used.
 */
export const DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdXGustoAPIVersion =
  {
    TwoThousandAndTwentyFiveMinus11Minus15: "2025-11-15",
  } as const;
/**
 * Determines the date-based API version associated with your API call. If none is provided, your application's [minimum API version](https://docs.gusto.com/embedded-payroll/docs/api-versioning#minimum-api-version) is used.
 */
export type DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdXGustoAPIVersion =
  ClosedEnum<
    typeof DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdXGustoAPIVersion
  >;

export type DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdRequest = {
  /**
   * Determines the date-based API version associated with your API call. If none is provided, your application's [minimum API version](https://docs.gusto.com/embedded-payroll/docs/api-versioning#minimum-api-version) is used.
   */
  xGustoAPIVersion?:
    | DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdXGustoAPIVersion
    | undefined;
  /**
   * The UUID of the employee
   */
  employeeId: string;
  /**
   * The UUID of the bank account
   */
  bankAccountUuid: string;
};

/** @internal */
export const DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdXGustoAPIVersion$outboundSchema:
  z.ZodMiniEnum<
    typeof DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdXGustoAPIVersion
  > = z.enum(
    DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdXGustoAPIVersion,
  );

/** @internal */
export type DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdRequest$Outbound =
  {
    "X-Gusto-API-Version": string;
    employee_id: string;
    bank_account_uuid: string;
  };

/** @internal */
export const DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdRequest$outboundSchema:
  z.ZodMiniType<
    DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdRequest$Outbound,
    DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdRequest
  > = z.pipe(
    z.object({
      xGustoAPIVersion: z._default(
        DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdXGustoAPIVersion$outboundSchema,
        "2025-11-15",
      ),
      employeeId: z.string(),
      bankAccountUuid: z.string(),
    }),
    z.transform((v) => {
      return remap$(v, {
        xGustoAPIVersion: "X-Gusto-API-Version",
        employeeId: "employee_id",
        bankAccountUuid: "bank_account_uuid",
      });
    }),
  );

export function deleteV1EmployeesEmployeeIdBankAccountsBankAccountIdRequestToJSON(
  deleteV1EmployeesEmployeeIdBankAccountsBankAccountIdRequest:
    DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdRequest,
): string {
  return JSON.stringify(
    DeleteV1EmployeesEmployeeIdBankAccountsBankAccountIdRequest$outboundSchema
      .parse(deleteV1EmployeesEmployeeIdBankAccountsBankAccountIdRequest),
  );
}
