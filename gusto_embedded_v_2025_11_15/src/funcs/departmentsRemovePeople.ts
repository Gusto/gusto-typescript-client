 ...
 */
export function departmentsRemovePeople(
...
> {
  const parsed = safeParse(
    request,
    (value) => PutRemovePeopleFromDepartmentRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["Department-People-Request-Body"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/departments/{department_uuid}/remove")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, PutRemovePeopleFromDepartmentResponse$inboundSchema, {
      key: "Department",
    }),
