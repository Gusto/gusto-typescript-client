...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.PostV1CompaniesAttachmentRequest$outboundSchema,
        value,
      ),
  ...
  appendForm(body, "category", payload.body.category);
  if (isBlobLike(payload.body.document)) {
    const file = payload.body.document;
    const blob = await normalizeBlob(file);
    const name = "name" in file ? (file.name as string) : undefined;
    appendForm(body, "document", blob, name);
  } else if (isReadableStream(payload.body.document.content)) {
    const buffer = await readableStreamToArrayBuffer(
      payload.body.document.content,
    );
    const contentType =
      getContentTypeFromFileName(payload.body.document.fileName)
      || "application/octet-stream";
    appendForm(
      body,
      "document",
      bytesToBlob(buffer, contentType),
      payload.body.document.fileName,
    );
  } else {
    const contentType =
      getContentTypeFromFileName(payload.body.document.fileName)
    ...
    appendForm(
      body,
      "document",
      bytesToBlob(payload.body.document.content, contentType),
      payload.body.document.fileName,
    );
  }
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, models.CompanyAttachment$inboundSchema),
