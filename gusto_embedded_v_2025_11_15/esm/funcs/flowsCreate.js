...
import { PostV1CompanyFlowsRequest$outboundSchema, PostV1CompanyFlowsResponse$inboundSchema, } from "../models/operations/postv1companyflows.js";
 ...
 * scope: `flows:write`
 */
export function flowsCreate(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => PostV1CompanyFlowsRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(201, PostV1CompanyFlowsResponse$inboundSchema, { key: "Flow" }), M.jsonErr(404, NotFoundErrorObject$inboundSchema), M.jsonErr(422, UnprocessableEntityErrorObject$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
