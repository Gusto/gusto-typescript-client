 ...
 */
export async function employeesGetAll(
  client: SDKCore,
  request: operations.GetV1CompaniesCompanyIdEmployeesRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<components.Employee>,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetV1CompaniesCompanyIdEmployeesRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.Employee$inboundSchema)),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
