 ...
 * scope: `payrolls:read`
 */
export async function wireInRequestsGetAll(
  client: SDKCore,
  request: operations.GetCompaniesCompanyUuidWireInRequestUuidRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<components.WireInRequest>,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  ...
  const path = pathToFunc("/v1/companies/{company_uuid}/wire_in_requests")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.WireInRequest$inboundSchema)),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
