...
async function run() {
  const result = await sdk.contractorPaymentMethods.getBankAccounts({
    contractorUuid: "<id>",
  });
...
import { contractorPaymentMethodsGetBankAccounts } from "openapi/funcs/contractorPaymentMethodsGetBankAccounts.js";
...
async function run() {
  const res = await contractorPaymentMethodsGetBankAccounts(sdk, {
    contractorUuid: "<id>",
  });
